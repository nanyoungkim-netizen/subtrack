...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 진단용: 마지막 웹훅 payload 저장(스레드 필드 확인용)
    try {
      await sql`CREATE TABLE IF NOT EXISTS app_settings (key text PRIMARY KEY, val text, updated_at timestamptz DEFAULT now())`;
      await sql`INSERT INTO app_settings (key,val,updated_at) VALUES ('last_webhook_payload', ${JSON.stringify(body).slice(0,6000)}, now())
        ON CONFLICT (key) DO UPDATE SET val=EXCLUDED.val, updated_at=now()`;
    } catch(_){}

    // 워크플로 스레드 정보(채널+ts). Zapier가 보내는 여러 필드명 대응
    const thread = {
      channel: body.channel || body.channel_id || body.channelId || body.channelID || '',
      ts: body.thread_ts || body.ts || body.message_ts || body.messageTs || body.event_ts || body.timestamp || ''
    };
